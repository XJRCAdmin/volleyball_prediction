...
int can_init(int& s, sockaddr_can* addr, ifreq* ifr, const char* canname) {
    if (canname == nullptr || strlen(canname) >= IFNAMSIZ) {
        std::cerr << "Error: Invalid CAN interface name." << std::endl;
        return -1;
    }
