...
static_assert(std::is_same_v<typename Executor::Input, start_point>, "");
static_assert(std::is_same_v<typename Executor::Output, Output>, "");
public:
...
static_assert(std::is_same_v<typename Executor::Input, Input>, "");
static_assert(std::is_same_v<typename Executor::Output, end_point>, "");
// static_assert(defaultConstructorTrait<Input>::value, "Input do not have an default constructor");
static_assert(defaultConstructorTrait<Executor>::value, "Executer do not have an default constructor");
static_assert(
    std::is_same_v<decltype(&Executor::process),
    void(Executor::*)(_SIMO<Executor, Input, end_point>& , Input&)>,
    "the signature of Executor::process is not right"
);
public:
    using iqueue_type = queue_buffer_mt<Input>;
    template<typename Executor_another>
    _SIMO(_SIMO<Executor_another> &pre_node, size_t threads_num);
    virtual ~_SIMO();
    void join();
...
template<typename Executor, typename Input>
template<typename Executor_another>
_SIMO<Executor, Input, end_point>::_SIMO(_SIMO<Executor_another> &pre_node, size_t threads_num)
    : _iqueue(pre_node._sub_one())
    , nthreads(0)
    , threadState(ThreadState::STOPED)
{
    static_assert(std::is_same_v<typename Executor_another::Output, Input>, "not same");
    for(size_t i = 0u; i < threads_num; i ++)
        threads.emplace_back(&_SIMO::handle, this);
}
...
template<typename Executor, typename Input>
int _SIMO<Executor, Input, end_point>::threads_num()
{
    return nthreads;
}
...
class _SIMO: 
    public _SIMO<Executor, start_point, Output>,
    public _SIMO<Executor, Input, end_point>
{
using Self = _SIMO<Executor, Input, Output>;
public:
    template<typename Executor_another>
    _SIMO(_SIMO<Executor_another> &pre_node, size_t threads_num, size_t buffer_size)
        : _SIMO<Executor, start_point, Output>(buffer_size)
        , _SIMO<Executor, Input, end_point>(pre_node, threads_num)
        {}
};
