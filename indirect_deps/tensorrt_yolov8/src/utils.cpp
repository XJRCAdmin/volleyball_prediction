...
void utils::show(const std::vector<std::vector<utils::Box>>& objectss, const std::vector<std::string>& classNames,
	const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch, const rs2::depth_frame& depth_frame)
{
	// todo
	...
	if (!imgsBatch[0].empty())
	{
		...
		// std::cout << imgsBatch[0].rows << " " << imgsBatch[0].cols << std::endl;
		int max_w = 960;
		int max_h = 540;
		/*int max_w = 720;
		int max_h = 405;*/
		if (imgsBatch[0].rows > max_h || imgsBatch[0].cols > max_w)
		{
			...
			cv::resizeWindow(windows_title, 640, 480);
				//max_w, imgsBatch[0].rows * max_w / imgsBatch[0].cols);
		}
	}
	...
	for (size_t bi = 0; bi < objectss.size(); bi++)
		...
		if (!objectss.empty())
		{
			for (auto& box : objectss[bi])
			{
				...
				if (classNames.size() == 1) // redball1
				{
					color = Colors::color1[box.label];
				}
				// ��ȡ���
				float depth_pixel[2] = { (box.left + box.right) / 2, (box.top + box.bottom) / 2 };
				float depth_value = depth_frame.get_distance(depth_pixel[0], depth_pixel[1]);
				...
				float camera_coordinate[3];
				rs2_deproject_pixel_to_point(camera_coordinate, &depth_intrin, depth_pixel, depth_value);
				camera_coordinate[1] = -camera_coordinate[1];
				//std::cout.precision(4);
				if (camera_coordinate[0] != 0 || camera_coordinate[1] != 0||camera_coordinate[2]!=0)
					std::cout << "Camera Coordinate (X, Y, Z): (" << camera_coordinate[0] << " m, " << camera_coordinate[1] << " m, " << camera_coordinate[2] << " m)" << std::endl;
